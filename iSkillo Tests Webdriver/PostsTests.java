...
class PostsTests {
	...
	void newPostVisibleOnHomepage() {
		...
		browseButton.click();
		browseButton.sendKeys("C:\\Users\\Nina\\Desktop\\cats.jpg");
		browseButton.submit();
	...
	void openingOfAvailablePosts() {
		...
		Login.navigateToLogin(driver);
		Login.login(driver, "nina777", "Nina777");
		List<WebElement> allposts = driver.findElements(By.xpath("//div[@class='row post-list-container']"));
		assertEquals(true, allposts.size());
		//posts.isSelected();
		//posts.isDisplayed();
		driver.close();
	...
	void LikePostFunctionality() {
		...
		Helpers.navigateToUrl(driver);
		Login.navigateToLogin(driver);
		WebElement firstPost = driver.findElement(By.xpath("//div[@class='row post-list-container']//div[1]//app-post-detail[1]//div[1]//div[2]//img[1]"));
		firstPost.click();
